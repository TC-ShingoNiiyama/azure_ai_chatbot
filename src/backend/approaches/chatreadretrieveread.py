...
load_dotenv() 
AZURE_MODEL_NAME = os.getenv("AZURE_OPENAI_EMB_MODEL_NAME")
AZURE_OPENAI_EMB_DEPLOYMENT = os.getenv("AZURE_OPENAI_EMB_DEPLOYMENT")
openai.api_type = os.getenv("AZURE_OPENAI_HOST")
openai.api_base = os.getenv("AZURE_OPENAI_EMB_ENDPOINT") 
openai.api_key = os.getenv("AZURE_OPENAI_EMB_API_KEY")
...
Each source has a name followed by colon and the actual information, always include the source name for each fact you use in the response. Use square brackets to reference the source, e.g. [info1.txt]. Don't combine sources, list each source separately, e.g. [info2.pdf].
"""
...
"""
    ...
    def get_gpt_embedding(text, model=AZURE_MODEL_NAME):
        print("aaa")
        print(text)
        response = openai.Embedding.create(engine=AZURE_MODEL_NAME, input=[text]).data[0].embedding
        print(response)
        return response
    ...
    def run(self, user_name: str, history: list[dict], overrides: dict) -> any:
        ...
        if has_vector:
            embedding_args = {"deployment_id": AZURE_OPENAI_EMB_DEPLOYMENT}
            embedding =  openai.Embedding.create(**embedding_args, model=AZURE_MODEL_NAME, input=history[-1]["user"])
