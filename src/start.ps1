...
Start-Process -FilePath $venvPythonPath -ArgumentList "-m pip --trusted-host pypi.python.org --trusted-host files.pythonhosted.org --trusted-host pypi.org install -r requirements.txt" -Wait -NoNewWindow
if ($LASTEXITCODE -ne 0) {
...
$port = 50505
$hostname = "localhost"
Start-Process -FilePath $venvPythonPath -ArgumentList "./app.py --port $port --host $hostname --reload" -Wait -NoNewWindow
